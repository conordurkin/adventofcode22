...
# Okay, now time to set up the grid itself - very large compared to the normal grid
x_list = list()
y_list = list()
...
x_range = np.arange(0, max(x_list)*2)
y_dim = max(y_list)
...
grid.loc[180] = '#' # This is our infinite floor at the bottom row - which is row 180 in the example
...
def try_sand_infinite(start_df):
    i = 0
